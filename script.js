...
function initializeSeats() {
    ...
    for (let i = 1; i <= totalSeats; i++) {
        ...
        seatDiv.className = 'seat cursor-pointer p-2 border rounded transition-colors';
        ...
        if (isAvailable) {
            ...
            seatDiv.className += ' bg-red-100 border-red-300 cursor-not-allowed opacity-50';
            seatDiv.innerHTML = `
...
function toggleSeatSelection(seatElement) {
    ...
    if (selectedSeats.includes(seatNumber)) {
        ...
        seatElement.className = seatElement.className.replace('bg-green-500 text-white', 'bg-gray-100 hover:bg-gray-200');
        seatElement.querySelector('img').src = seatElement.querySelector('img').src.replace('seat-green-filled.png', 'seat-gray.png');
        ...
        seatElement.className = seatElement.className.replace('bg-gray-100 hover:bg-gray-200', 'bg-green-500 text-white');
        seatElement.querySelector('img').src = seatElement.querySelector('img').src.replace('seat-gray.png', 'seat-green-filled.png');
...
function calculateTotalPrice() {
    ...
    const promoInput = document.querySelector('input[placeholder="Have any coupon?"]');
    if (promoInput) {
        promoInput.value = '';
    }
}
...
function updateSeatCounter() {
    ...
    if (seatLeftElement) {
        ...
        if (seatsLeftDiv) {
            if (seatsLeft === 0) {
                seatsLeftDiv.className = seatsLeftDiv.className.replace('bg-green-100 text-green-700', 'bg-red-100 text-red-700');
                seatLeftElement.textContent = 'No';
            } else if (seatsLeft <= 2) {
                seatsLeftDiv.className = seatsLeftDiv.className.replace('bg-green-100 text-green-700', 'bg-yellow-100 text-yellow-700');
                seatsLeftDiv.className = seatsLeftDiv.className.replace('bg-red-100 text-red-700', 'bg-yellow-100 text-yellow-700');
            } else {
                seatsLeftDiv.className = seatsLeftDiv.className.replace('bg-yellow-100 text-yellow-700', 'bg-green-100 text-green-700');
                seatsLeftDiv.className = seatsLeftDiv.className.replace('bg-red-100 text-red-700', 'bg-green-100 text-green-700');
            }
        }
    }
}
...
function applyPromoCode() {
    ...
    if (discount > 0) {
        ...
        alert(`Promo code applied! You saved ${discount} BDT (${discountPercent}% discount)`);
    }
}

// Show/Hide coupon section
function hideCouponSection() {
    const couponDiv = document.querySelector('.bg-white.rounded-xl.text-center.flex.items-center');
    if (couponDiv) {
...
function showDiscountInfo(percent, amount) {
    ...
    const grandTotalDiv = document.getElementById('grand-total').closest('.flex.justify-between.font-semibold.mt-4');
...
function updateNextButton() {
    ...
    const passengerName = document.querySelector('input[placeholder="Enter your name"]');
    ...
    if (hasSeats && hasName && hasPhone) {
        ...
        nextButton.className = nextButton.className.replace('bg-green-600 cursor-not-allowed opacity-50', 'bg-green-600 hover:bg-green-700 cursor-pointer');
    } else {
        nextButton.disabled = true;
        nextButton.className = nextButton.className.replace('bg-green-600 hover:bg-green-700 cursor-pointer', 'bg-green-600 cursor-not-allowed opacity-50');
    }
}
...
function initializeEventListeners() {
    ...
    const applyButton = document.querySelector('.bg-green-500.text-white.px-4.py-2.rounded-r-full');
    if (applyButton) {
...
function handleNextButton() {
    const passengerName = document.querySelector('input[placeholder="Enter your name"]').value.trim();
    ...
    const bookingData = {
        ...
        email: email,
        totalAmount: document.getElementById('grand-total').textContent.replace('BDT ', '')
    };
